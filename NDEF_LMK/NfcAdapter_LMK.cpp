...
NfcAdapter_LMK::NfcAdapter_LMK(PN532Interface_LMK &interface)
{
    shield = new PN532_LMK(interface);
}

NfcAdapter_LMK::~NfcAdapter_LMK(void)
{
    delete shield;
}

void NfcAdapter_LMK::begin(boolean verbose)
{
    shield->begin();
...
boolean NfcAdapter_LMK::tagPresent(unsigned long timeout)
{
    uint8_t success;
    uidLength = 0;
...
boolean NfcAdapter_LMK::write(NdefMessage& ndefMessage)
{
    boolean success;
    uint8_t type = guessTagType();
...
unsigned int NfcAdapter_LMK::guessTagType()
{
