...
#include "Arduino.h"
#include "PN532_LMK.h"
#include "PN532_debug_LMK.h"
#include <string.h>

#define HAL(func)   (_interface->func)

PN532_LMK::PN532_LMK(PN532Interface_LMK &interface)
{
...
*/
/**************************************************************************/
void PN532_LMK::PrintHex(const uint8_t *data, const uint32_t numBytes)
{
#ifdef ARDUINO
...
*/
/**************************************************************************/
void PN532_LMK::PrintHexChar(const uint8_t *data, const uint32_t numBytes)
{
#ifdef ARDUINO
    for (uint8_t i = 0; i < numBytes; i++) {
        if (data[i] < 0x10) {
...
*/
/**************************************************************************/
uint32_t PN532_LMK::getFirmwareVersion(void)
{
    uint32_t response;

    pn532_packetbuffer[0] = PN532_LMK_COMMAND_GETFIRMWAREVERSION;
...
/**************************************************************************/
/*!
    @brief  Read a PN532_LMK register.
...
*/
/**************************************************************************/
uint32_t PN532_LMK::readRegister(uint16_t reg)
{
    uint32_t response;

    pn532_packetbuffer[0] = PN532_LMK_COMMAND_READREGISTER;
...
/**************************************************************************/
/*!
    @brief  Write to a PN532_LMK register.
...
*/
/**************************************************************************/
uint32_t PN532_LMK::writeRegister(uint16_t reg, uint8_t val)
{
    uint32_t response;

    pn532_packetbuffer[0] = PN532_LMK_COMMAND_WRITEREGISTER;
    pn532_packetbuffer[1] = (reg >> 8) & 0xFF;
    pn532_packetbuffer[2] = reg & 0xFF;
...
/**************************************************************************/
/*!
    Writes an 8-bit value that sets the state of the PN532_LMK's GPIO pins
    ...
    @warning This function is provided exclusively for board testing and
             ...
             will be required to recover the PN532_LMK.
...
*/
/**************************************************************************/
bool PN532_LMK::writeGPIO(uint8_t pinstate)
{
    // Make sure pinstate does not try to toggle P32 or P34
    pinstate |= (1 << PN532_LMK_GPIO_P32) | (1 << PN532_LMK_GPIO_P34);

    // Fill command buffer
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_WRITEGPIO;
    pn532_packetbuffer[1] = PN532_LMK_GPIO_VALIDATIONBIT | pinstate;  // P3 Pins
...
/**************************************************************************/
/*!
    Reads the state of the PN532_LMK's GPIO pins
...
*/
/**************************************************************************/
uint8_t PN532_LMK::readGPIO(void)
{
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_READGPIO;
...
*/
/**************************************************************************/
bool PN532_LMK::SAMConfig(void)
{
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_SAMCONFIGURATION;
...
*/
/**************************************************************************/
bool PN532_LMK::setPassiveActivationRetries(uint8_t maxRetries)
{
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_RFCONFIGURATION;
...
bool PN532_LMK::setRFField(uint8_t autoRFCA, uint8_t rFOnOff)
{
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_RFCONFIGURATION;
...
*/
/**************************************************************************/
bool PN532_LMK::readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout)
{
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_INLISTPASSIVETARGET;
...
*/
/**************************************************************************/
bool PN532_LMK::mifareclassic_IsFirstBlock (uint32_t uiBlock)
{
...
*/
/**************************************************************************/
bool PN532_LMK::mifareclassic_IsTrailerBlock (uint32_t uiBlock)
{
    // Test if we are in the small or big sectors
    if (uiBlock < 128)
...
/*!
    ...
    INDATAEXCHANGE command.  See section 7.3.8 of the PN532_LMK User Manual
...
*/
/**************************************************************************/
uint8_t PN532_LMK::mifareclassic_AuthenticateBlock (uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
    uint8_t i;
    ...
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_INDATAEXCHANGE;   /* Data Exchange Header */
...
*/
/**************************************************************************/
uint8_t PN532_LMK::mifareclassic_ReadDataBlock (uint8_t blockNumber, uint8_t *data)
{
    DMSG("Trying to read 16 bytes from block ");
    DMSG_INT(blockNumber);

    /* Prepare the command */
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_INDATAEXCHANGE;
...
*/
/**************************************************************************/
uint8_t PN532_LMK::mifareclassic_WriteDataBlock (uint8_t blockNumber, uint8_t *data)
{
...
*/
/**************************************************************************/
uint8_t PN532_LMK::mifareclassic_FormatNDEF (void)
{
...
*/
/**************************************************************************/
uint8_t PN532_LMK::mifareclassic_WriteNDEFURI (uint8_t sectorNumber, uint8_t uriIdentifier, const char *url)
{
...
*/
/**************************************************************************/
uint8_t PN532_LMK::mifareultralight_ReadPage (uint8_t page, uint8_t *buffer)
{
    if (page >= 64) {
...
*/
/**************************************************************************/
uint8_t PN532_LMK::mifareultralight_WritePage (uint8_t page, uint8_t *buffer)
{
    /* Prepare the first command */
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_INDATAEXCHANGE;
    pn532_packetbuffer[1] = 1;                           /* Card number */
...
*/
/**************************************************************************/
bool PN532_LMK::inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
    uint8_t i;

    pn532_packetbuffer[0] = 0x40; // PN532_LMK_COMMAND_INDATAEXCHANGE;
...
/**************************************************************************/
/*!
    @brief  'InLists' a passive target. PN532_LMK acting as reader/initiator,
            peer acting as card/responder.
*/
/**************************************************************************/
bool PN532_LMK::inListPassiveTarget()
{
    pn532_packetbuffer[0] = PN532_LMK_COMMAND_INLISTPASSIVETARGET;
    pn532_packetbuffer[1] = 1;
...
int8_t PN532_LMK::tgInitAsTarget(const uint8_t* command, const uint8_t len, const uint16_t timeout){
 ...
 * Peer to Peer
 */
int8_t PN532_LMK::tgInitAsTarget(uint16_t timeout)
{
    const uint8_t command[] = {
        PN532_LMK_COMMAND_TGINITASTARGET,
        0,
        0x00, 0x00,         //SENS_RES
...
int16_t PN532_LMK::tgGetData(uint8_t *buf, uint8_t len)
{
    buf[0] = PN532_LMK_COMMAND_TGGETDATA;
...
bool PN532_LMK::tgSetData(const uint8_t *header, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
    if (hlen > (sizeof(pn532_packetbuffer) - 1)) {
        ...
        pn532_packetbuffer[0] = PN532_LMK_COMMAND_TGSETDATA;
...
int16_t PN532_LMK::inRelease(const uint8_t relevantTarget){

    pn532_packetbuffer[0] = PN532_LMK_COMMAND_INRELEASE;
...
/**************************************************************************/
/*!
    @brief  Poll FeliCa card. PN532_LMK acting as reader/initiator,
            peer acting as card/responder.
...
*/
/**************************************************************************/
int8_t PN532_LMK::felica_Polling(uint16_t systemCode, uint8_t requestCode, uint8_t * idm, uint8_t * pmm, uint16_t *systemCodeResponse, uint16_t timeout)
{
  pn532_packetbuffer[0] = PN532_LMK_COMMAND_INLISTPASSIVETARGET;
  pn532_packetbuffer[1] = 1;
...
*/
/**************************************************************************/
int8_t PN532_LMK::felica_SendCommand (const uint8_t *command, uint8_t commandlength, uint8_t *response, uint8_t *responseLength)
{
...
*/
/**************************************************************************/
int8_t PN532_LMK::felica_RequestService(uint8_t numNode, uint16_t *nodeCodeList, uint16_t *keyVersions)
{
...
*/
/**************************************************************************/
int8_t PN532_LMK::felica_RequestResponse(uint8_t * mode)
{
  uint8_t cmd[9];
...
*/
/**************************************************************************/
int8_t PN532_LMK::felica_ReadWithoutEncryption (uint8_t numService, const uint16_t *serviceCodeList, uint8_t numBlock, const uint16_t *blockList, uint8_t blockData[][16])
{
...
*/
/**************************************************************************/
int8_t PN532_LMK::felica_WriteWithoutEncryption (uint8_t numService, const uint16_t *serviceCodeList, uint8_t numBlock, const uint16_t *blockList, uint8_t blockData[][16])
{
...
*/
/**************************************************************************/
int8_t PN532_LMK::felica_RequestSystemCode(uint8_t * numSystemCode, uint16_t *systemCodeList)
{
  uint8_t cmd[9];
...
*/
/**************************************************************************/
int8_t PN532_LMK::felica_Release()
{
  // InRelease
  pn532_packetbuffer[0] = PN532_LMK_COMMAND_INRELEASE;
