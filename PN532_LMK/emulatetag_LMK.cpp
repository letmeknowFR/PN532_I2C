...
bool EmulateTag_LMK::init(){
  pn532.begin();
  return pn532.SAMConfig();
}

void EmulateTag_LMK::setNdefFile(const uint8_t* ndef, const int16_t ndefLength){
...
void EmulateTag_LMK::setUid(uint8_t* uid){
  uidPtr = uid;
}

bool EmulateTag_LMK::emulate(const uint16_t tgInitAsTargetTimeout){
...
void EmulateTag_LMK::setResponse(responseCommand cmd, uint8_t* buf, uint8_t* sendlen, uint8_t sendlenOffset){
  switch(cmd){
